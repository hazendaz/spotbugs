...
/**
 * Boolean analysis properties for use in the AnalysisContext. These can be used to enable or disable various analysis
 * features in the bytecode analysis framework.
 *
...
public interface AnalysisFeatures {
    ...
    /**
     * Determine (1) what exceptions can be thrown on exception edges, (2) which catch blocks are reachable, and (3)
     * which exception edges carry only "implicit" runtime exceptions.
     */
    ...
    /**
     * In the null pointer analysis, track value numbers that are known to be null. This allows us to not lose track of
     * null values that are not currently in the stack frame but might be in a heap location where the value is
     * recoverable by redundant load elimination or forward substitution.
     */
    public static final @AnalysisFeature int TRACK_VALUE_NUMBERS_IN_NULL_POINTER_ANALYSIS = 6;

    /**
     * Merge similar warnings. If we are tracking warnings across versions, it is useful to merge all similar issues
     * together. Otherwise, when we compare the warnings in two different versions, we will not be able to match them up
     * correctly.
     */
    public static final @AnalysisFeature int MERGE_SIMILAR_WARNINGS = 7;

    /**
     * Number of boolean analysis properties reserved for the bytecode analysis framework. Clients of the framework may
     * use property values &gt;= this value.
     *
