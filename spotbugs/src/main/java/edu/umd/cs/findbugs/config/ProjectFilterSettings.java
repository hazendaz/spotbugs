...
public class ProjectFilterSettings implements Cloneable {
    ...
    public boolean displayWarning(BugInstance bugInstance) {
        ...
        if (!displayFalseWarnings) {
            boolean isFalseWarning = !Boolean.valueOf(bugInstance.getProperty(BugProperty.IS_BUG, "true"))
                    .booleanValue();
            if (isFalseWarning) {
                return false;
            }
     ...
     *
     * Note that bug categories that are not explicity hidden will appear active even if they are not members of this
     * set.
     *
