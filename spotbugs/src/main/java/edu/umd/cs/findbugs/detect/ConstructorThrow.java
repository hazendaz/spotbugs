...
public class ConstructorThrow extends OpcodeStackDetector {
    ...
    /**
     * 1. Check for any throw expression in the constructor. 2. Check for any unchecked exception throw inside
     * constructor, or any of the called methods. If the class is final, we are fine, no finalizer attack can happen. In
     * the first pass the detector shouldn't report, because there could be a final finalizer and a throwing
     * constructor. Reporting in this case would be a false positive as classes with a final finalizer are not
    ...
    /**
     * Reports ContructorThrow bug if there is an unhandled unchecked exception thrown directly or indirectly from the
     * currently visited method.
     *
     * If the exception is thrown directly, the bug is reported at the throw. If the exception is thrown indirectly
     * (through a method call), the bug is reported at the call of the method which throws the exception.
     */
    ...
    private void addToExHandlesToMethodCallsByMethodsMap(String containerMethod, String calledMethod,
        ...
        if (exHandlesToMethodCallsByMethodsMap.containsKey(containerMethod)) {
            ...
            exHandlesToMethodCallsByMethodsMap.put(containerMethod, new HashMap<String, Set<String>>() {
                {
     ...
     * be called from method call opcode. Works for invokedynamic as well.
     *
     * @param seen
     *            the opcode @see #sawOpcode(int)
     *
