...
public class DateFormatStringChecker extends OpcodeStackDetector {
    ...
    /**
     * Contains special flags that can trigger check (triggers); property of the rule (isRequired - whether flag should
     * be contained or be absent); flags to be checked (checkItems) and a bypassing flag (ignoreFlag - whether a rule
     * can be skipped if such flag is included)
     */
    private static final class Rule {
        ...
        /**
         * dateFormat is being checked for existence of any keywords (triggering flags) to start the check (if
         * ignoreFlag flag was found - further checking is skipped); if isRequired property is: - true: dateFormat is
         * checked for existence of flags (this.flags) - false: dateFormat is checked for absence of flags (this.flags)
         */
    ...
    public void sawOpcode(int seen) {
        ...
        int idx = -1;
        /*
         * idx represents the position of the string in the stack, the value is 0 in cases when it has only 1 parameter
         * provided; 1 in cases when there are multiple parameters given; -1 when no method found, or has different
         * number of parameters within the specified functions (given in switch and if)
         */
        if (("java/text/SimpleDateFormat".equals(cl) && "(Ljava/lang/String;)V".equals(si)
                && (Const.CONSTRUCTOR_NAME.equals(nm) || "applyPattern".equals(nm)
                        || "applyLocalizedPattern".equals(nm)))
            ...
            idx = 0;
                ...
                || ("org/apache/commons/lang3/time/FastDateFormat".equals(cl) && "getInstance".equals(nm)
                        && ("(Ljava/lang/String;Ljava/util/Locale;)Lorg/apache/commons/lang3/time/FastDateFormat;"
                                .equals(si)
                                || "(Ljava/lang/String;Ljava/util/TimeZone;)Lorg/apache/commons/lang3/time/FastDateFormat;"
                                        .equals(si)))) {
            idx = 1;
        }
        if (idx == -1) {
    ...
    private boolean runDateFormatRuleVerify(String dateFormat) {
        ...
        return Stream.of(
                ...
                .anyMatch(rule -> rule.verify(interpretedDateFormat));
    }
