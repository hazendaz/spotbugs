...
public class FindRefComparison implements Detector, ExtendedTypes {
    ...
    public MethodDescriptor getInvokedMethod(ConstantPoolGen cpg, InvokeInstruction inv) {
        ...
        String methodName = inv.getMethodName(cpg);
        String methodSig = inv.getSignature(cpg);
        MethodDescriptor invokedMethod = DescriptorFactory.instance().getMethodDescriptor(
                ClassName.toSlashedClassName(invoked), methodName, methodSig, inv instanceof INVOKESTATIC);
