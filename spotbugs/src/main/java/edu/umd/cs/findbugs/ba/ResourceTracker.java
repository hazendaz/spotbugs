...
/**
 * A ResourceTracker is used with ResourceValueAnalysis to determine where in a method a certain kind of resource is
 * created, and to model the effect of instructions on the state of that resource.
 *
...
public interface ResourceTracker<Resource> {
     ...
     *
     * @return an opaque Resource object if it is a creation site, or null if it is not a creation site
     */
     ...
     * @param frame
     *            the ResourceValueFrame representing the stack prior to executing the instruction
     *
    ...
    public boolean isResourceClose(BasicBlock basicBlock, InstructionHandle handle, ConstantPoolGen cpg,
            Resource resource, ResourceValueFrame frame) throws DataflowAnalysisException;
     ...
     * This allows different resource types to be tracked with varying precision. For example, we might want to ignore
     * implicit exceptions for stream objects, but treat them as significant for database resources.
     *
     * @param resource
    ...
    /**
     * Determine whether the analysis should ignore given exception edge. This allows the analysis to customize which
     * kinds of exceptions are significant.
     *
     * @param edge
     *            the exception edge
     * @param resource
    ...
    /**
     * Return if the given parameter slot contains the resource instance upon entry to the method. This is for resources
