...
public class SwitchFallthrough extends OpcodeStackDetector implements StatelessDetector {
    ...
    public void sawOpcode(int seen) {
        ...
        if (isSwitch(seen) || isReturn(seen) || (isBranch(seen) && isBranchTargetAfterNextCaseOffset())) {
            clearAllDeadStores();
        }
     ...
     * end of the last case.
     *
     * @return <code>true</code> if the branch target is after the next switch offset as it is the case for a switch
     *         break.
     */
