...
/**
 * This detector finds all the methods of a subclass which are hiding the static methods of the superclass. Please
 * see @see <a href=
 * "https://wiki.sei.cmu.edu/confluence/display/java/MET07-J.+Never+declare+a+class+method+that+hides+a+method+declared+in+a+superclass+or+superinterface">SEI
 * CERT MET07-J</a>
 *
...
public class FindHiddenMethod implements Detector {
    ...
    /*
     * Only the super classes are analyzed, not the super interfaces Reason: when I looked in detail of the super
     * interfaces, I found there is no point to analyze as whenever we have method hiding due to super interfaces, We
     * can call the method by fully qualified name only (as Interface can't be instantiated). Calling a hidden method by
     * fully qualified name is a compliant test. See @see <a href=
     ...
     * >SEI CERT MET07-J</a> Thereof, it is useless to analyze super interfaces in the detector.
     */
    @Override
    public void visitClassContext(ClassContext classContext) {
        ...
        for (Method method : methods) {
            ...
            if (!method.isStatic() || method.isPrivate() || method.isSynthetic() || MemberUtils.isMainMethod(method)
            ...
            for (JavaClass superClass : superClasses) {
                ...
                if (Arrays.asList(superMethods).contains(method)) {
                    bugReporter.reportBug(new BugInstance(this, "HSM_HIDING_METHOD", NORMAL_PRIORITY).addClass(subClass)
                            .addMethod(subClass, method).addClass(superClass).describe(ClassAnnotation.SUPERCLASS_ROLE)
    ...
    /**
     * This method checks for the inner class exceptional cases. As whenever there an inner class, '.access$' methods
     * are created hiddenly to access the outer class attributes.
     */
    ...
    /**
     * This method checks for the autoGenerated methods for an inner class - exceptional cases. As whenever there an
     * inner class, '.class$' methods are created hiddenly.
     */
