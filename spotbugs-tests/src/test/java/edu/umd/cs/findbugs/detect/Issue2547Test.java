...
public class Issue2547Test extends AbstractIntegrationTest {
    ...
    private void assertBug(int num) {
        BugInstanceMatcher bugTypeMatcher = new BugInstanceMatcherBuilder().bugType("RV_EXCEPTION_NOT_THROWN").build();
        assertThat(getBugCollection(), containsExactly(num, bugTypeMatcher));
    }

    private void assertBug(String method, int line) {
        BugInstanceMatcher bugTypeMatcher = new BugInstanceMatcherBuilder().bugType("RV_EXCEPTION_NOT_THROWN")
                .inClass("Issue2547").inMethod(method).atLine(line).build();
