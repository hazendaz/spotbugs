...
public abstract class AbstractIntegrationTest {
    ...
    private static final List<Path> BUILD_CLASS_SEARCH_PREFIXES = List.of(
            ...
            Path.of("build/classes/java/main"), Path.of("build/classes/groovy/main"),
    ...
    /**
     * Asserts that there are exactly zero bug instances with the given bug type. The match is checked according to
     * {@link BugInstanceMatcher}.
     *
     * @param bugType
    ...
    /**
     * Asserts that there are exactly the given number of bug instances with the given bug type. The match is checked
     * according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param count
    ...
    protected final void assertBugTypeCount(String bugType, int count) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).build();
        assertThat(getBugCollection(), containsExactly(count, matcher));
    }

    /**
     * Asserts that there are at least {@code min} and at most {@code max} number of bug instances (both inclusive) with
     * the given bug type. The match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param min
     *            the expected minimal number of matches (inclusive)
     * @param max
     *            the expected maximal number of matches (inclusive)
     */
    ...
    /**
     * Asserts that there are exactly zero bug instances with the given bug type in the given class. The match is
     * checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
     *            the expected class name. See
     *            {@link BugInstanceMatcher#BugInstanceMatcher(String, String, String, String, String, Integer, Integer, Confidence, String, List)
     *            BugInstanceMatcher's constructor} for details.
     */
    ...
    /**
     * Asserts that there are exactly the given number of bug instances with the given bug type in the given class. The
     * match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
    ...
    protected final void assertBugInClassCount(String bugType, String className, int count) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).inClass(className).build();
        assertThat(getBugCollection(), containsExactly(count, matcher));
    }

    /**
     * Asserts that there is a bug instance with the given bug type in the given class. The match is checked according
     * to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
     ...
     *            BugInstanceMatcher's constructor} for details.
     * @param method
     *            the expected method's name
     */
    protected final void assertNoBugInMethod(String bugType, String className, String method) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).inClass(className)
                .inMethod(method).build();

        assertThat(getBugCollection(), containsExactly(0, matcher));
    }

    /**
     * Asserts that there are exactly the given number of bug instances with the given bug type in the given method. The
     * match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
    ...
    /**
     * Asserts that there is a bug instance with the given bug type in the given class in the given method. The match is
     * checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
    ...
    /**
     * Asserts that there is a bug instance with the given bug type in the given class in the given method at the given
     * line. The match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
    ...
    /**
     * Asserts that there is a bug instance with the given bug type in the given class in the given method at the given
     * line with the given confidence. The match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
     ...
     */
    protected final void assertBugInMethodAtLineWithConfidence(String bugType, String className, String method,
            int line, Confidence confidence) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).inClass(className)
                .inMethod(method).atLine(line).withConfidence(confidence).build();
        assertThat(getBugCollection(), hasItem(matcher));
    }

    /**
     * Asserts that there is a bug instance with the given bug type in the given class in the given method at the given
     * field. The match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
    ...
    protected final void assertNoBugAtField(String bugType, String className, String fieldName) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).inClass(className)
                .atField(fieldName).build();
        assertThat(getBugCollection(), containsExactly(0, matcher));
    }

    /**
     * Asserts that there is a bug instance with the given bug type in the given class at the given field. The match is
     * checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
    ...
    /**
     * Asserts that there is a bug instance with the given bug type in the given class at the given field at the given
     * line. The match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
    ...
    protected final void assertBugAtFieldAtLine(String bugType, String className, String fieldName, int line) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).inClass(className)
                .atField(fieldName).atLine(line).build();
        assertThat(getBugCollection(), hasItem(matcher));
    }

    /**
     * Asserts that there is a bug instance with the given bug type in the given class in the given method at the given
     * variable at the given line. The match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
     ...
     *            the expected line
     */
    protected final void assertBugAtVar(String bugType, String className, String method, String variableName,
            int line) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).inClass(className)
                .inMethod(method).atVariable(variableName).atLine(line).build();
        assertThat(getBugCollection(), hasItem(matcher));
    }

    /**
     * Asserts that there is a bug instance with the given bug type in the given class in the given method at the given
     * variable. The match is checked according to {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
     * @param className
     ...
     *            the expected variable name
     */
    protected final void assertBugInMethodAtVariable(String bugType, String className, String method,
            String variableName) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).inClass(className)
                .inMethod(method).atVariable(variableName).build();
        assertThat(getBugCollection(), hasItem(matcher));
    }

    /**
     * Asserts that there is a bug instance with the given bug type at the given line. The match is checked according to
     * {@link BugInstanceMatcher}.
     *
     * @param bugType
     *            the expected bug type
    ...
    protected final void assertBugAtLine(String bugType, int line) {
        final BugInstanceMatcher matcher = new BugInstanceMatcherBuilder().bugType(bugType).atLine(line).build();
        assertThat(getBugCollection(), hasItem(matcher));
    }
}
